...
import { getAllBooksByPage } from "../../../api/book-service";
...
const Books = () => {
  ...
  const loadData = async () => {
    try {
      const resp = await getAllBooksByPage();
  ...
  const handlePage = (bookId) => {
    navigate(`/budak/books/${bookId}`);
  };
  useEffect(() => {
    loadData(0);
  }, []);
        ...
        <>
          <Row className="g-5 book">
            {books.map((book, index) => (
              <Col key={index}>
                <Book {...book} handlePage={handlePage} />
              </Col>
            ))}
          </Row>
